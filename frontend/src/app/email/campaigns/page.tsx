...
export default function EmailCampaignsPage() {
  ...
  const loadCampaigns = async () => {
    ...
    setErrorBanner(null);
    try {
      const data = await campaignsApi.getAll({ type: 'email' });
      setCampaigns(data || []);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Send bulk personalized emails with throttle limits, unsubscribe compliance, and open tracking.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl overflow-hidden shadow-2xs">
        {loading ? (
          <div className="p-12 text-center text-xs text-slate-400">Loading email campaigns...</div>
        ) : campaigns.length === 0 ? (
          <div className="p-12 text-center space-y-3">
            ...
            <div className="text-sm font-bold text-slate-800 dark:text-slate-200">No Email Campaigns Found</div>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              Schedule your first targeted outbound email campaign with account rotation and spam rate protection.
            </p>
            <Link
              href="/campaigns/new"
            ...
            {campaigns.map((camp) => (
              <div key={camp._id} className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                ...
                <div className="flex items-center gap-6 text-xs font-medium">
                  ...
                  <div className="text-center">
                    <div className="text-slate-400 text-[10px] uppercase">Failed / Bounced</div>
