...
export default function WhatsAppTemplatesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <select
            ...
            className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-white border border-slate-200 text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 shadow-2xs"
          >
          ...
          {activeTab === 'official' && (
            <button
              onClick={handleSync}
              disabled={syncing}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors shadow-2xs"
      ...
      {successToast && (
        <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-800 animate-in fade-in">
      ...
      {activeTab === 'official' && (
        <div className="space-y-4">
          <div className="bg-emerald-50/60 border border-emerald-200/70 rounded-xl p-3.5 flex items-start gap-2.5 text-xs text-emerald-900">
            <ShieldCheck className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5" />
            <div className="leading-relaxed">
              <strong>Meta WhatsApp Business Platform Rules:</strong> Official Cloud API templates must be pre-approved by Meta before broadcasting. Variables use numbered tokens like <code className="bg-emerald-100 px-1 py-0.5 rounded font-mono">{'{{1}}'}</code>, <code className="bg-emerald-100 px-1 py-0.5 rounded font-mono">{'{{2}}'}</code>.
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-3">
              {loading ? (
                ...
                <div className="p-12 text-center space-y-3 bg-white rounded-2xl border border-slate-200 shadow-2xs">
                  <div className="w-12 h-12 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto">
                    <FileCode className="w-6 h-6" />
                  </div>
                  <div className="text-sm font-bold text-slate-800">No Meta Templates Found</div>
                  ...
                  {templates.map((tmpl) => (
                    ...
                    >
                      ...
                      <div className="text-[11px] text-slate-500 mb-2">
                        Category: <span className="font-medium text-slate-700">{tmpl.category || 'MARKETING'}</span> • Lang: <span className="font-medium text-slate-700">{tmpl.language || 'en_US'}</span>
                      </div>
            ...
            <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-2xs space-y-4 h-fit sticky top-4">
              ...
              {selectedTemplate ? (
                <div className="space-y-3">
                  <div className="bg-[#efeae2]/40 p-4 rounded-2xl border border-slate-200 space-y-2">
                    <div className="text-[10px] font-bold text-emerald-800 uppercase tracking-wider">
                      WhatsApp Bubble Preview
                    </div>
                    <div className="bg-white border border-slate-200 text-slate-900 p-3.5 rounded-2xl rounded-tl-none text-xs leading-relaxed shadow-2xs">
                  ...
                  <div className="space-y-1.5 text-xs text-slate-600">
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      <span className="text-slate-400">Template ID / Name:</span>
                      <span className="font-mono font-bold text-slate-800">{selectedTemplate.name}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      <span className="text-slate-400">Category:</span>
                      <span className="font-semibold text-slate-800">{selectedTemplate.category || 'MARKETING'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      <span className="text-slate-400">Language:</span>
                      <span className="font-semibold text-slate-800">{selectedTemplate.language || 'en_US'}</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-1.5 py-2 text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl transition-colors"
                  >
      ...
      {activeTab === 'regular' && (
        <div className="space-y-4">
          <div className="bg-blue-50/60 border border-blue-200/70 rounded-xl p-3.5 flex items-start gap-2.5 text-xs text-blue-900">
            <Zap className="w-4 h-4 text-blue-600 flex-shrink-0 mt-0.5" />
            <div className="leading-relaxed">
              <strong>Regular WhatsApp Snippets:</strong> These quick replies & broadcast templates execute immediately over your linked WhatsApp Baileys connection without Meta approval delay. Named tokens like <code className="bg-blue-100 px-1 py-0.5 rounded font-mono">{'{{name}}'}</code>, <code className="bg-blue-100 px-1 py-0.5 rounded font-mono">{'{{company}}'}</code> are auto-merged from your contact list.
            </div>
          </div>
            ...
            <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-2xs space-y-4 h-fit sticky top-4">
              ...
              {selectedSnippet ? (
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5 text-xs text-slate-600">
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      <span className="text-slate-400">Snippet Name:</span>
                      <span className="font-bold text-slate-800">{selectedSnippet.name}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-slate-100">
                      <span className="text-slate-400">Variables:</span>
                      <div className="flex gap-1 flex-wrap">
                        {selectedSnippet.variables.map((v) => (
                          <span key={v} className="bg-slate-100 px-1.5 py-0.5 rounded font-mono text-[10px]">
                            {`{{${v}}}`}
                          </span>
                        ))}
                      </div>
